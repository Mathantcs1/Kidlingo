...
function processChain(
  ...
  function process(opts: YahooOption[], type: "CALL" | "PUT") {
    for (const o of opts) {
      ...
      if (volOi < 0.8) continue; // filter low vol/OI
      const premium = vol * o.lastPrice * 100;
      if (premium < 50000) continue; // min $50K premium
      const ivRank = calcIvRank(iv, allIvs);
      const dte = calcDTE(o.expiration);
      // Classify sweep vs block by size
    ...
    watchlist.map(async (symbol) => {
      try {
        ...
        const allIvs = [...calls, ...puts]
          ...
          .filter((v) => v > 0);
        const trades = processChain(symbol, calls, puts, allIvs);
...
export async function GET(req: NextRequest) {
  ...
  try {
    allTrades = await fetchRealUnusualTrades();
    usingReal = allTrades.length > 0;
  } catch {
    allTrades = [];
  }

  if (!usingReal) allTrades = SIM_TRADES;
